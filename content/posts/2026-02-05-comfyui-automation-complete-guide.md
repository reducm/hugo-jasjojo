...
description = "深度解析 ComfyUI 的 API 架构，提供完整的自动化调用方案、参数调整、输出获取代码示例。解决 JSON 参数手动调整的痛点。包含自动生成的目录，方便长文章浏览。"
...
> 你有很多 ComfyUI 工作流 JSON 文件，很多地方都要手动调整参数？每次生成都要手动点"执行"？这篇指南将带你从手动操作走向完全自动化。包含自动生成的目录，方便长文章浏览。

## 目录

- [问题：手动操作的痛点](#问题手动操作的痛点)
- [核心问题：ComfyUI 有 API 吗？](#核心问题comfyui-有-api-吗)
- [第一部分：理解 ComfyUI 的 API 架构](#第一部分：理解comfyui-的api-架构)
- [第二部分：获取生成的文件](#第二部分：获取生成的文件)
- [第三部分：自动化参数调整](#第三部分：自动化参数调整)
- [第四部分：完整的自动化示例](#第四部分：完整的自动化示例)
- [第五部分：批量生成变体](#第五部分：批量生成变体)
- [第六部分：高级技巧](#第六部分：高级技巧)
- [第七部分：推荐的工具和资源](#第七部分：推荐的工具和资源)
- [第八部分：从手动到自动化的跨越](#第八部分：从手动到自动化的跨越)
- [总结](#总结)
